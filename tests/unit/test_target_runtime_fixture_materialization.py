...
RUN_ID = "run-daacs-runtime-06"


def _runner_plan_hash() -> str:
    return stable_contract_hash(
        {
            "purpose": "target runtime fixture materialization",
...
def _preflight(run_id: str = RUN_ID) -> dict:
    request = TargetRuntimePreflightRequest(
        ...
        metadata={
            "raw_prompt": "DAACS_FIXTURE_MATERIALIZATION_RAW_PROMPT_SENTINEL",
            "runtime_payload": "DAACS_FIXTURE_MATERIALIZATION_RUNTIME_SENTINEL",
        },
    )
...
def _admission_result(run_id: str = RUN_ID) -> dict:
    ...
    request = TargetRuntimeAdapterAdmissionRequest(
        ...
        metadata={
            "raw_file_body": "DAACS_FIXTURE_MATERIALIZATION_ADMISSION_RAW_SENTINEL",
        },
    )
...
def _output_manifest_read_model(run_id: str = RUN_ID):
    ...
    manifest = build_disabled_target_runtime_output_manifest(
        request=TargetRuntimeOutputManifestRequest(
            ...
            metadata={
                "raw_file_body": "DAACS_FIXTURE_MATERIALIZATION_MANIFEST_RAW_SENTINEL",
            },
        )
    ).to_dict()
...
def _bundle(run_id: str = RUN_ID) -> dict:
    ...
    return build_disabled_target_runtime_generated_artifact_bundle(
        request=TargetRuntimeGeneratedArtifactBundleRequest(
            ...
            metadata={
                "raw_file_body": "DAACS_FIXTURE_MATERIALIZATION_BUNDLE_RAW_SENTINEL",
            },
        )
    ).to_dict()


def _request(tmp_path, **overrides) -> TargetRuntimeFixtureMaterializationRequest:
    bundle = _bundle()
    fields = {
        "run_id": RUN_ID,
        "runner_plan_hash": _runner_plan_hash(),
        ...
        "workspace_root": tmp_path / "fixture-workspace",
        "metadata": {
            "raw_file_body": "DAACS_FIXTURE_MATERIALIZATION_REQUEST_RAW_SENTINEL",
            "runtime_payload": "DAACS_FIXTURE_MATERIALIZATION_REQUEST_PAYLOAD_SENTINEL",
        },
    }
    fields.update(overrides)
    return TargetRuntimeFixtureMaterializationRequest(**fields)
...
def test_target_runtime_fixture_materialization_writes_sanitized_run_workspace_files(
    tmp_path,
):
    result = materialize_target_runtime_fixture_artifacts(
        request=_request(tmp_path)
    ).to_dict()
    serialized = _serialized(result)

    assert result["projection_version"] == TARGET_RUNTIME_FIXTURE_MATERIALIZATION_VERSION
    assert result["mode"] == TARGET_RUNTIME_FIXTURE_MATERIALIZATION_MODE
    assert result["status"] == "passed"
    assert result["reason"] == "target_runtime_fixture_artifacts_materialized"
    assert result["counts"]["generated_artifact_bundle_hash_match_count"] == 1
    assert result["counts"]["fixture_artifact_record_count"] == 3
    assert result["counts"]["fixture_artifact_content_hash_count"] == 3
    assert result["counts"]["fixture_workspace_file_write_count"] == 3
    assert result["counts"]["artifact_content_public_return_count"] == 0
    assert result["execution_boundary"]["fixture_workspace_file_write_count"] == 3
...
def test_target_runtime_fixture_materialization_blocks_missing_bundle_projection(
    ...
    assert result["counts"]["fixture_workspace_file_write_count"] == 0
    assert not (tmp_path / "fixture-workspace").exists()


def test_target_runtime_fixture_materialization_blocks_bundle_hash_mismatch(tmp_path):
    result = materialize_target_runtime_fixture_artifacts(
        request=_request(
            tmp_path,
...
def test_target_runtime_fixture_materialization_blocks_unsafe_run_id(tmp_path):
    bundle = _bundle()
    result = materialize_target_runtime_fixture_artifacts(
...
def test_target_runtime_fixture_materialization_exposes_no_raw_body_or_source(
    tmp_path,
):
    result = materialize_target_runtime_fixture_artifacts(
        request=_request(tmp_path)
    ...
    file_text = "\n".join(
        (
            tmp_path / "fixture-workspace" / record["workspace_relative_path"]
        ).read_text(encoding="utf-8")
        for record in result["artifact_records"]
    )

    for forbidden in (
        "DAACS_FIXTURE_MATERIALIZATION_RAW_PROMPT_SENTINEL",
        "DAACS_FIXTURE_MATERIALIZATION_RUNTIME_SENTINEL",
        "DAACS_FIXTURE_MATERIALIZATION_ADMISSION_RAW_SENTINEL",
        "DAACS_FIXTURE_MATERIALIZATION_MANIFEST_RAW_SENTINEL",
        "DAACS_FIXTURE_MATERIALIZATION_BUNDLE_RAW_SENTINEL",
        "DAACS_FIXTURE_MATERIALIZATION_REQUEST_RAW_SENTINEL",
        "DAACS_FIXTURE_MATERIALIZATION_REQUEST_PAYLOAD_SENTINEL",
        "raw_prompt",
        "runtime_payload",
        "raw_file_body",
        "source_code",
...
def test_target_runtime_fixture_materialization_does_not_spawn_read_env_or_call_network(
    tmp_path,
    monkeypatch,
):
