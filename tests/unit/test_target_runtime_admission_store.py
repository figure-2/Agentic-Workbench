...
RUN_ID = "run-daacs-runtime-02"


def _runner_plan_hash() -> str:
    return stable_contract_hash(
        {
            "purpose": "target runtime adapter admission persistence",
...
def _preflight(run_id: str = RUN_ID) -> dict:
    request = TargetRuntimePreflightRequest(
        ...
        metadata={
            "raw_prompt": "DAACS_RUNTIME_STORE_RAW_PROMPT_SENTINEL",
            "runtime_payload": "DAACS_RUNTIME_STORE_PAYLOAD_SENTINEL",
        },
    )
...
def _admission_result(run_id: str = RUN_ID) -> dict:
    ...
    request = TargetRuntimeAdapterAdmissionRequest(
        ...
        metadata={
            "raw_file_body": "DAACS_RUNTIME_STORE_RAW_FILE_SENTINEL",
        },
    )
...
def test_target_runtime_adapter_admission_record_is_hash_status_count_only():
    record = target_runtime_adapter_admission_record_from_result(_admission_result())
    serialized = _serialized(record.to_dict())

    assert record.status == "blocked"
    assert record.reason == "target_runtime_adapter_disabled"
    assert record.adapter_reach_count == 1
    assert record.adapter_disabled_block_count == 1
    ...
    for forbidden in (
        "DAACS_RUNTIME_STORE_RAW_PROMPT_SENTINEL",
        "DAACS_RUNTIME_STORE_PAYLOAD_SENTINEL",
        "DAACS_RUNTIME_STORE_RAW_FILE_SENTINEL",
        "raw_prompt",
        "runtime_payload",
        "raw_file_body",
        "provider_payload",
...
def test_target_runtime_adapter_admission_sqlite_persists_only_hash_status_count_rows(
    tmp_path,
):
    store = SQLiteTargetRuntimeAdmissionStore(tmp_path / "admission-store")
    ...
    assert len(rows) == 1
    assert set(row) == TARGET_RUNTIME_ADMISSION_EXPECTED_COLUMNS[
        "target_runtime_adapter_admissions"
    ]
    assert row["adapter_admission_hash"] == record.adapter_admission_hash
    assert row["preflight_hash"] == record.preflight_hash
    assert "DAACS_RUNTIME_STORE_RAW_PROMPT_SENTINEL" not in serialized_row
    assert "DAACS_RUNTIME_STORE_PAYLOAD_SENTINEL" not in serialized_row
    assert "DAACS_RUNTIME_STORE_RAW_FILE_SENTINEL" not in serialized_row
    ...
    assert set(read_model["adapter_admissions"][0]) == {
        "status",
        "reason",
        "runner_plan_hash",
        "preflight_hash",
...
def test_target_runtime_adapter_admission_sqlite_duplicate_rolls_back_partial_row(
    tmp_path,
):
    store = SQLiteTargetRuntimeAdmissionStore(tmp_path / "admission-store")
    repository = store.repository()
...
def test_target_runtime_adapter_admission_read_model_blocks_bad_sqlite_stores(tmp_path):
    ...
    (corrupted_root / TARGET_RUNTIME_ADMISSION_DB_NAME).write_text(
        "not sqlite",
        encoding="utf-8",
    )
    wrong_schema_root = tmp_path / "wrong-schema"
    wrong_schema_root.mkdir()
    with sqlite3.connect(wrong_schema_root / TARGET_RUNTIME_ADMISSION_DB_NAME) as conn:
        conn.execute("CREATE TABLE target_runtime_adapter_admissions (run_id TEXT)")
        conn.commit()

    unavailable = target_runtime_adapter_admission_public_read_model_from_sqlite(
        unavailable_root,
        run_id=RUN_ID,
    )
    corrupted = target_runtime_adapter_admission_public_read_model_from_sqlite(
        corrupted_root,
        run_id=RUN_ID,
    )
    wrong_schema = target_runtime_adapter_admission_public_read_model_from_sqlite(
        wrong_schema_root,
        run_id=RUN_ID,
    )
    ...
    assert unavailable["counts"]["adapter_admission_record_count"] == 0
    assert corrupted["counts"]["adapter_admission_record_count"] == 0
    assert wrong_schema["counts"]["adapter_admission_record_count"] == 0


def test_target_runtime_adapter_admission_store_does_not_read_env_or_call_network(
    monkeypatch,
    tmp_path,
):
