...
`AW-LIVE-46` adds a disabled first-call execution capsule authz final
authorization. The authorization requires an execution capsule authz release
seal hash and a separate expected authz release seal hash match. It binds authz
release-seal, final-authorization, claim-boundary, and no-call counter hashes
into one status/reason/hash/count projection. The authorization still reports
`execution_capsule_authz_final_authz_execution_closed` and keeps
`execution_permission_count=0`.

`AW-LIVE-47` adds a disabled first-call execution capsule authz final
authorization export/read-model. The export requires an execution capsule authz
final-authorization hash and a separate expected authz final-authorization hash
match. It binds authz final-authorization, export metadata, claim-boundary,
and no-call counter hashes into one status/reason/hash/count projection. The
...
`AW-LIVE-49` adds a disabled first-call execution capsule authz final
authorization operator review. The review requires an execution capsule authz
final-authorization handoff packet hash and a separate expected handoff hash
match. It binds authz final-authorization handoff, operator-review,
claim-boundary, and no-call counter hashes into one status/reason/hash/count
projection. The review still reports
...
`AW-LIVE-55` adds a disabled first-call execution capsule authz final
authorization final authorization handoff packet. The packet requires the
execution capsule authz final-authorization final authorization export hash and
a separate expected export hash match. It binds the export, export read-model,
claim-boundary, and no-call counter hashes into one status/reason/hash/count
...
`AW-LIVE-58` adds a disabled first-call execution capsule authz final
authorization final authorization release attestation. The attestation requires
the execution capsule authz final-authorization final authorization operator
decision hash and a separate expected decision hash match. It binds the
operator-decision, release-attestation, claim-boundary, and no-call counter
...
`AW-LIVE-65` adds the disabled first-call execution capsule authz final
authorization final authorization final authorization release attestation. The
attestation requires the operator decision hash from `AW-LIVE-64` and a
matching expected decision hash. It binds operator-decision,
...
`AW-LIVE-67` adds the disabled first-call execution capsule authz final
authorization final authorization final authorization final authorization. The
authorization requires the release-seal hash from `AW-LIVE-66` and a matching
expected release-seal hash. It binds release-seal, final-authorization,
claim-boundary, and no-call counter hashes into one status/reason/hash/count
projection while keeping `execution_permission_count=0`.
...
- execution capsule authz release seals must bind to execution capsule authz
  ...
  hashes only and must not grant execution permission.
- execution capsule authz final authorization must bind to execution capsule
  authz release seal, final-authorization material, claim-boundary, and no-call
  counter hashes only and must not grant execution permission.
...
- execution capsule authz final authorization handoff packets must bind to
  ...
  claim-boundary, and no-call counter hashes only and must not grant execution
  permission.
- execution capsule authz final authorization operator reviews must bind to
