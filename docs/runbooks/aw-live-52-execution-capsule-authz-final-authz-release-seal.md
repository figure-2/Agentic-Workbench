# AW-LIVE-52 Runbook: Execution Capsule Authz Final Authz Release Seal Boundary

## Conclusion

The execution capsule authz final-authorization release seal is a blocked
no-call record after the execution capsule authz final-authorization release
attestation. It shows that a future first-call authorization capsule can
represent a release seal over attestation evidence without opening the provider
path.

## Execution Capsule Authz Final Authz Release Seal Projection

The public execution capsule authz final-authorization release seal returns
only:
...
```json
{
  ...
  "execution_capsule_authz_final_authz_release_seal_hash": "<hash>",
...
|---|---|
| execution capsule authz final authorization release attestation | hash is present and execution remains closed |
| expected execution capsule authz final authorization release attestation hash | equals computed release-attestation hash |
| execution capsule authz final authorization release seal payload | present |
...
- Execution capsule authz final authorization release attestation hash is
...
- Supplied seal upstream hash does not match the computed release-attestation
  hash.
- Seal material details are missing.
...
The execution capsule authz final-authorization release seal is not
provider-result evidence. It is only local no-call evidence that a future
authorization capsule can represent a release seal over attestation evidence
