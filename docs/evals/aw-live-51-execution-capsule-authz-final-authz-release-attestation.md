# AW-LIVE-51 Execution Capsule Authz Final Authz Release Attestation Boundary

## Conclusion

`AW-LIVE-51` adds a disabled first-call execution capsule authorization final
authorization release attestation after `AW-LIVE-50`. The release attestation
binds the authz final-authorization operator-decision hash, release-attestation
...
- disabled execution capsule authz final-authorization release attestation
  projection
- expected execution capsule authz final-authorization operator decision hash
  match gate
...
| Gate | Result |
|---|---|
| execution capsule authz final-authorization operator decision hash exists | covered |
...
```powershell
python -m pytest tests\integration\test_api_public_projection.py::test_provider_envelope_precheck_api_blocks_execution_capsule_authz_final_authz_release_attestation_without_expected_decision_hash tests\integration\test_api_public_projection.py::test_provider_envelope_precheck_api_blocks_execution_capsule_authz_final_authz_release_attestation_without_attestation_payload tests\integration\test_api_public_projection.py::test_provider_envelope_precheck_api_builds_execution_capsule_authz_final_authz_release_attestation_but_keeps_execution_disabled tests\smoke\test_local_service_demo.py::test_local_service_demo_can_include_provider_envelope_precheck_without_calls tests\unit\test_public_claim_projection_docs.py -q
...
```text
AW-LIVE-51 adds a local disabled execution capsule authorization final
authorization release attestation that binds authz final-authorization
...
```text
AW-LIVE-51 must not be described as a provider outcome, provider response,
...
- Public output excludes raw prompt, provider body, provider payload,
  ...
  operator identity.
- Missing expected execution capsule authz final-authorization operator
  decision hash or missing release attestation payload blocks the boundary.
...
- The release attestation is not provider-result evidence. A later
