# AW-LIVE-29 Operator Decision Packet Boundary

## Conclusion

`AW-LIVE-29` adds a disabled first-call operator decision packet after the
operator handback. The packet binds the operator handback hash,
operator-decision hash, claim-boundary hash, and no-call counter hash into one
public projection.
...
- disabled operator decision packet public projection
- expected operator handback hash match gate
- operator decision packet payload presence gate
...
| Gate | Result |
|---|---|
| operator handback hash exists | covered |
...
| decision packet still keeps execution permission `0` | covered |
...
```powershell
python -m pytest tests\integration\test_api_public_projection.py::test_provider_envelope_precheck_api_blocks_operator_decision_packet_without_expected_handback_hash tests\integration\test_api_public_projection.py::test_provider_envelope_precheck_api_blocks_operator_decision_packet_without_packet_payload tests\integration\test_api_public_projection.py::test_provider_envelope_precheck_api_builds_operator_decision_packet_but_keeps_execution_disabled tests\smoke\test_local_service_demo.py::test_local_service_demo_can_include_provider_envelope_precheck_without_calls -q
...
```text
AW-LIVE-29 adds a local disabled operator decision packet that binds handback,
...
```text
AW-LIVE-29 invokes a provider call, records a provider response, grants
execution permission, verifies a live operator decision, or validates model
quality.
```
...
- The operator decision packet is a public projection over sanitized no-call
  evidence.
- Public output excludes raw prompt, provider body, provider payload,
  ...
  operator identity.
- Missing expected operator handback hash or missing operator decision packet
  payload blocks the packet.
- A complete packet still reports `operator_decision_packet_execution_closed`
...
- The operator decision packet is not provider-result evidence. A later
