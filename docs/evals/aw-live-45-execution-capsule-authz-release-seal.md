# AW-LIVE-45 Execution Capsule Authz Release Seal Boundary

## Conclusion

`AW-LIVE-45` adds a disabled first-call execution capsule authorization release
seal after the execution capsule authz release attestation. The release seal
binds the authz release-attestation hash, seal-material hash,
...
- disabled execution capsule authz release seal public projection
- expected execution capsule authz release attestation hash match gate
- execution capsule authz release seal payload presence gate
...
| Gate | Result |
|---|---|
| execution capsule authz release attestation hash exists | covered |
...
| seal request is represented as a count only | covered |
...
```powershell
python -m pytest tests\integration\test_api_public_projection.py::test_provider_envelope_precheck_api_blocks_execution_capsule_authz_release_seal_without_expected_attestation_hash tests\integration\test_api_public_projection.py::test_provider_envelope_precheck_api_blocks_execution_capsule_authz_release_seal_without_seal_payload tests\integration\test_api_public_projection.py::test_provider_envelope_precheck_api_builds_execution_capsule_authz_release_seal_but_keeps_execution_disabled tests\smoke\test_local_service_demo.py::test_local_service_demo_can_include_provider_envelope_precheck_without_calls -q
...
```text
AW-LIVE-45 adds a local disabled execution capsule authorization release seal
that binds authz release-attestation, seal-material, claim-boundary, and
...
```text
AW-LIVE-45 invokes a provider call, records a provider response, grants
...
- The execution capsule authz release seal is a public projection over
...
- Public output excludes raw prompt, provider body, provider payload,
  ...
  operator identity.
- Missing expected execution capsule authz release attestation hash or missing
  release seal payload blocks the boundary.
...
- The execution capsule authz release seal is not provider-result evidence. A
